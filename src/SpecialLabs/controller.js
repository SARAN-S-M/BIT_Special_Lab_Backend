...
exports.studentInterviewRequest = async (req, res) => {
    // console.log('studentInterviewRequest');
    try {
        const studentEmail = req.userEmail;
        ...
        let lab = await SpecialLab.findOne({ specialLabCode });

        if (!lab) {
            return res.status(404).json({ error: 'Lab not found' });
        }

        // console.log('lab', lab.StudentInterviewRequests);

        lab.StudentInterviewRequests.push({
...
exports.GetstudentInterviewRequest = async (req, res) => {
    try {
        const facultyEmail = req.userEmail;
        ...
        let studentInterviewRequests = existingSpecialLab.StudentInterviewRequests;

        let newstudentInterviewRequests = studentInterviewRequests.map(student => {
            return {
            HashedId: hashId(student._id, process.env.SECRET_KEY),
        ...
        res.status(200).json({ students: newstudentInterviewRequests });
...
exports.changeLabDateTime = async (req, res) => {
    try {
        const facultyEmail = req.userEmail;
        ...
        const { studentEmail, hashedStudentId, newDate, newTime } = req.body;
        ...
        // Find the student with the matching hash
        const studentIndex = existingSpecialLab.StudentInterviewRequests.findIndex(student => hashId(student._id, secretKey) === hashedStudentId);
        ...
        existingSpecialLab.StudentInterviewRequests[studentIndex].InterviewDate = new Date(`${newDate}T${newTime}:00`);
        existingSpecialLab.StudentInterviewRequests[studentIndex].InterviewTime = new Date(`${newDate}T${newTime}:00`);

        await existingSpecialLab.save();

        res.status(200).json({ message: 'Date and time updated successfully' });
    }
    catch (error) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
...
exports.changeLabResult = async (req, res) => {
    try {
        const facultyEmail = req.userEmail;
        ...
        const { hashedStudentId, result } = req.body;
        const secretKey = process.env.SECRET_KEY;

        // console.log(hashedStudentId, result);
        ...
        // If the result is "Selected", update the user model of the student with the new lab
        if (result === "Selected") {
            let student = await User.findOne({ email: existingSpecialLab.StudentInterviewRequests[studentIndex].studentEmail });

            if (student) {
                student.specialLabCode = specialLabCode;
                student.specialLab = existingSpecialLab.specialLabName;
                await student.save();
            }
        }

        // Remove the student from the list
        existingSpecialLab.StudentInterviewRequests.splice(studentIndex, 1);
